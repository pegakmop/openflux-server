...
встроен прямо в бинарник) отдаётся по адресу `/admin/`. Это тонкий клиент поверх того же JSON API
`/v1/admin/*`, что описан ниже: один раз вставляете `CONTROLPLANE_ADMIN_TOKEN` (хранится в
`localStorage` браузера), дальше создаёте/просматриваете/включаете/отключаете ноды, ключи и
ingest-токены через вкладки вместо curl. `deploy/install.sh` ставит перед ним Nginx с сертификатом
Let's Encrypt.
